...
app.get(
  ...
  (req, res) => {
    try {
      if (req.user?.needs_language_setup) {
        console.log("Redirecting user to /select-language for setup.");
        return res.redirect(`${CLIENT_URL}/select-language`);
      }
      console.log("User authenticated and does not need language setup.");
